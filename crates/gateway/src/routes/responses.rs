...
pub async fn handler(
    ...
    // 协议转换：OpenAI Responses → Unified
    let converter = OpenAIResponses;
    ...
    let decision = state.router.route(&unified, &context).await.map_err(|e| {
        (StatusCode::SERVICE_UNAVAILABLE, Json(json!({"error": format!("routing failed: {}", e)})))
    })?;

    // 获取提供商适配器
    let adapter = state.router.registry.get(&decision.provider).ok_or_else(|| {
    ...
    let response = adapter.chat(&unified).await.map_err(|e| {
        ...
        (StatusCode::BAD_GATEWAY, Json(json!({"error": format!("provider error: {}", e)})))
    })?;
    ...
    // 协议转换：Unified → OpenAI Responses
    let output = converter.from_unified_response(&response).map_err(|e| {
        (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": format!("conversion error: {}", e)})))
    })?;

    let value: Value = serde_json::from_slice(&output).map_err(|e| {
        (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": format!("json error: {}", e)})))
    })?;
